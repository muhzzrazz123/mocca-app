  ...
  return (
    <header className="h-16 lg:h-20 bg-white/95 dark:bg-[#11212D]/95 backdrop-blur border-b border-[#9BA8AB]/40 dark:border-[#253745] px-4 lg:px-8 flex items-center justify-between gap-4 sticky top-0 z-30 select-none transition-colors duration-200">
        ...
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <h1 className="text-base lg:text-xl font-black text-[#06141B] dark:text-[#CCD0CF] tracking-tight flex items-center gap-2 font-serif">
              ...
              {activeTab === 'dashboard' && userMode !== 'staff' && (
              ...
              {userMode === 'staff' && (
                <span className="inline-flex items-center gap-1 text-[11px] font-bold text-emerald-800 dark:text-emerald-300 bg-emerald-500/15 px-2 py-0.5 rounded-full border border-emerald-500/30">
                  <ReceiptText size={12} className="text-emerald-600 dark:text-emerald-400" /> Cashier Mode
                </span>
              )}
            </h1>
            <p className="text-[11px] text-[#4A5C6A] dark:text-[#9BA8AB] font-medium hidden sm:block">
              {userMode === 'staff' ? (
                <>MOCCA Counter • {todayFormatted} • Cashier: <strong className="text-emerald-600 dark:text-emerald-400">{activeStaffName}</strong></>
              ) : (
      ...
      <div className="flex items-center gap-2 lg:gap-3">
        {/* Quick Search - Only shown for Owner */}
        {userMode !== 'staff' && (
          <button
        ...
        {/* Period Filter Dropdown - Owner Only */}
        {userMode !== 'staff' && (
        ...
        {/* Lock Screen Button */}
        <button
          onClick={() => lock(userMode === 'staff' ? 'staff' : 'owner')}
          ...
          title={userMode === 'staff' ? 'Lock Counter' : 'Lock App (Owner PIN Protected)'}
        >
          <Lock size={16} />
        </button>

        {/* Quick Daily Entry Action Button - Owner Only */}
        {userMode !== 'staff' && (
          <button
        ...
        {userMode !== 'staff' && activeTab !== 'billing' && (
          ...
          >
            <ReceiptText size={15} className="stroke-[2.5]" />
            <span>New Bill</span>
          </button>
        )}
      </div>
